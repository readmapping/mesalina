...
//void Boundaries::setValues(long queryBegin, long queryEnd, long refBegin, long refEnd) {
void Boundaries::setValues(long refBegin, long refEnd, long queryBegin, long queryEnd) {
  this->refB = refBegin;
  this->refE = refEnd;
  this->queryB =queryBegin;
  this->queryE = queryEnd;
}
...
int DynProg::updateMatrix(const Dp_type& type){
    ...
    if(scores.openGap != 0){//affine gap penalties
        ...
        for(int i = 1; i <= L1; i++){
            ...
            LEFT[0][i] = M[0][i];// + scores.openGap; // Or without openGap ?
        }
        for(int i = 1; i <= L2; i++){
            UP[i][0] = M[i][0];//+ scores.openGap;// Or without openGap ?
...
int DynProg::dpFillStatic(const string& ref, const string& query, bool forward,
    ...
    long idx2, colRB;
    for(long i = 1; i <= L2; i++){
      idx2 = forward ? i-bandL : L1 - L2 + i - bandL;
      colRB = forward ? i+bandR : L1 - L2 + i + bandR;

      if(idx2>0){//left value undefined

            if (left){
              d = scores.scoreMatrix[ref[offset.refB+idx2-1]][query[offset.queryB+i-1]];
//              cout << ref[offset.refB+idx2-1];
            }

            else {
              d = scores.scoreMatrix[ref[offset.refE-idx2]][query[offset.queryE-i]];
//              cout << ref[offset.refB+L1-idx2+1];
            }
        ...
        for(long j = max( 1L, idx2); j <= min((long)L1, colRB-1); j++ ){
            ...
            if(left)
              ...
              d = scores.scoreMatrix[ref[offset.refE-j]][query[offset.queryE-i]];
...
int DynProg::dpFillStatic(const string& ref, const string& query, bool forward,
  ...
  return dpFillStatic(ref, query, forward, offset, type, true);
}
...
int DynProg::dpTraceBackStatic(int& i, int& j, const Dp_type& type, Dp_output& output,
    ...
    while( (i > qBound || j > rBound) && (!local || M[ i ][ j ] > 0)){
        if( j > 0 && i > 0 && M[ i ][ j ] == M[ i-1 ][ j-1 ] + scores.match && ref[offset.refB+j-1] == query[offset.queryB+i-1]){
            debug_tr(cerr << i<< ", "<<j<<": " << ref[offset.refB + j - 1] << " = "<< query[offset.queryB+i-1] <<endl);
            ss << '=';
            i-- ;  j-- ;
        }
        else if(j > 0 && i > 0 && M[ i ][ j ] == M[ i-1 ][ j-1 ] + scores.mismatch){
            debug_tr(cerr << i<< ", "<<j<<": " << ref[offset.refB + j - 1] << " X "<< query[offset.queryB+i-1]<<endl);
            ss << 'X';
            output.editDist++;
            i-- ;  j-- ;
        }
        else if(j > 0 && j > i+bandCorrection - bandL && M[ i ][ j ] == LEFT[ i ][ j ]){
            debug_tr(cerr << i<< ", "<<j<<": " << ref[offset.refB + j - 1]<< " D ."<<endl);
            ss << 'D';
            output.editDist++;
            j-- ;
        }
        else if(i > 0 && j < i+bandCorrection + bandR && M[ i ][ j ] == UP[ i ][ j ]){
            debug_tr(cerr << i<< ", "<<j<<": . I "<< query[offset.queryB+i-1] <<endl);
            ss << 'I';
            output.editDist++;
            i-- ;
        }
...
int DynProg::dpTraceBackStatic(int& i, int& j, const Dp_type& type, Dp_output& output,
        std::stringstream & ss, const Boundaries& offset, bool forward, const string& ref, const string& query, bool left){
    //TODO: after mallocs: change this traversal to row-order
    //change stringstream to char array of length dimensions array
    int qBound = 0; int rBound = 0;
    int bandL = max(bandLeft, bandSize);
...
//        cout << "i > 0" << i > 0 ? "true": "false" <<endl;
//        cout <<  "M[ i ][ j ] == M[ i-1 ][ j-1 ] + scores.match" << M[ i ][ j ] == M[ i-1 ][ j-1 ] + scores.match ? "true": "false" <<endl;
//        cout << "left ? ref[offset.refB+j-1] == query[offset.queryB+i-1] : ref[offset.refE-j] == query[offset.queryE-i]"
        if( j > 0 && i > 0 && M[ i ][ j ] == M[ i-1 ][ j-1 ] + scores.match &&
                (left ? ref[offset.refB+j-1] == query[offset.queryB+i-1] : ref[offset.refE-j] == query[offset.queryE-i])){
            debug_tr(left ? cerr << i<< ", "<<j<<":\t" << ref[offset.refB + j - 1] << " = "<< query[offset.queryB+i-1] <<endl :
                            cerr << i<< ", "<<j<<":\t" << ref[offset.refE - j ]    << " = "<< query[offset.queryE-i] <<endl);
            ss << '=';
            i-- ;  j-- ;
        }
        else if(j > 0 && i > 0 && M[ i ][ j ] == M[ i-1 ][ j-1 ] + scores.mismatch){
            debug_tr(left ? cerr << i<< ", "<<j<<":\t" << ref[offset.refB + j - 1] << " X "<< query[offset.queryB+i-1] <<endl :
                            cerr << i<< ", "<<j<<":\t" << ref[offset.refE - j]     << " X "<< query[offset.queryE-i] <<endl);
            ss << 'X';
            output.editDist++;
            i-- ;  j-- ;
        }
        else if(j == 0 || (i > 0 && j < i+bandCorrection + bandR && M[ i ][ j ] == UP[ i ][ j ])){
            debug_tr(left ? cerr << i<< ", "<<j<<":\t. I "<< query[offset.queryB+i-1] <<endl :
                            cerr << i<< ", "<<j<<":\t. I "<< query[offset.queryE-i] <<endl);
            ss << 'I';
            output.editDist++;
            i-- ;
        }
        else if(i == 0 || (j > 0 && j > i+bandCorrection - bandL && M[ i ][ j ] == LEFT[ i ][ j ])){
            debug_tr(left ? cerr << i<< ", "<<j<<":\t" << ref[offset.refB + j - 1]<< " D ."<<endl :
                            cerr << i<< ", "<<j<<":\t" << ref[offset.refE - j]<< " D ."<<endl);
            ss << 'D';
            output.editDist++;
            j-- ;
        }
            ...
            if(j > 0 && i > 0){
                ...
                if (left)
                  ...
                  cerr << "Characters "    << ref[offset.refE-j] << "  " << query[offset.queryE-i] << endl;
            }
            cerr << "BandSize " << max(max(bandSize, bandL), bandRight) << endl;
            if(j > 0)
                cerr << "Scores: Left "    << M[ i ][ j-1 ] << endl;
            if(i > 0)
                cerr << "Scores: Up "    << M[ i-1 ][ j ] << endl;
            cerr << "bandCorrection: " << bandCorrection <<endl;
            cerr << "bandL: " << bandL << endl;
            cerr << "LEFT[ i ][ j ]: " << LEFT[ i ][ j ] <<endl;
            exit(1);
        }
    }
    return 0;
...
void  DynProg::print_matrices(const string& ref,
  ...
  print_matrices(ref, query, offset, gapMatrices, true);
}

void  DynProg::print_matrices(const string& ref,
        const string& query, const Boundaries& offset, bool gapMatrices, bool left )
{
        cout << "  ";
