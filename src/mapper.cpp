...
bool compAlignmentScore(const Alignment_t::Ptr i,const Alignment_t::Ptr j){
...
bool compPairScore(const Alignment_t::Ptr i,const Alignment_t::Ptr j){
...
//    else
//    {
//        sa.MEM(P, matches, min_len, maxBranchWidth, false);
//        if(tryHarder){//TODO: change try-harder to recalculate only after forward + reverse has been tried
//            //easy solution: try reverse and if found: skip
//            if(matches.empty())
//                sa.MEM(P, matches, min_len, 1000, false);
//            if(matches.empty())
//                sa.MEM(P, matches, 20, 1000, false);
//        }        
//    }
    postProcess(matches);
}
...
Alignment_t::Ptr extendAlignment(DynProg& dp_, const string& S, const string& P,
...
void inexactMatch(const SparseSA& sa, DynProg& dp_, Read_t & read,const align_opt & alnOptions, bool fwStrand, bool print){
    ...
    if(matches.size()>0){
        ...
        while(alnCount < max(1,alnOptions.alignmentCount/2) &&
            ...
            Alignment_t::Ptr alignment = extendAlignment(dp_, sa.S, P, matches, begin, end, editDist, alnOptions);
            if(alignment!=NULL){
                if(!fwStrand){
...
void unpairedMatch(const SparseSA& sa, DynProg& dp_, Read_t & read,const align_opt & alnOptions, bool print){
    ...
    if(matches.size()>0 || matchesRC.size()>0){
        ...
        while(alnCount < alnOptions.alignmentCount &&
            ...
            Alignment_t::Ptr alignment = extendAlignment(dp_, sa.S, lisIntervals[lisIndex].fw ? P : Prc, *matchVector, begin, end, editDist, alnOptions);
            if(alignment!=NULL){
...
//PAIRED END OPTIONS
bool isConcordant(const Alignment_t::Ptr mate1, const Alignment_t::Ptr mate2, const paired_opt& options){
    bool firstLeft = true;
...
void setUnPaired(Alignment_t::Ptr toset, Read_t& read, bool upstream){
...
void setPaired(Alignment_t::Ptr mate1, Alignment_t::Ptr mate2, Read_t& upstream, Read_t& downstream, bool concordant){
...
void pairedMatch1(const SparseSA& sa, DynProg& dp_, Read_t & mate1, Read_t & mate2, const align_opt & alnOptions, const paired_opt & pairedOpt, bool print){
    ...
    if(alnCount1>0 && alnCount2>0){
        ...
        while(i < alnCount1 && alnCount < alnOptions.alignmentCount){
            Alignment_t::Ptr aln1 = mate1.alignments[i];
            aln1->setLocalPos(sa);
            int j = 0;
            while(j < alnCount2 && alnCount < alnOptions.alignmentCount){
                Alignment_t::Ptr aln2 = mate2.alignments[j];
        ...
        if(pairedOpt.discordant){
            ...
            while(i < discordantAln.size() && alnCount < alnOptions.alignmentCount){
                Alignment_t::Ptr aln1 = mate1.alignments[discordantAln[i].mate1];
                Alignment_t::Ptr aln2 = mate2.alignments[discordantAln[i].mate2];
...
bool alignFromLIS(const SparseSA& sa, DynProg& dp_, Read_t& read, Lis_t & lis, long editDist, const align_opt & alnOptions){
    ...
    //sort this candidate region by query position
    sort(matchVector->begin()+begin,matchVector->begin()+end+1, compMatchesQuery);
    Alignment_t::Ptr alignment = extendAlignment(dp_, sa.S, lis.fw ? read.sequence : read.rcSequence, *matchVector, begin, end, editDist, alnOptions);
    if(alignment!=NULL){
        lis.extended = true;
        if(!lis.fw){
            alignment->flag.set(4,true);
...
void unpairedMatchFromLIS(const SparseSA& sa, DynProg& dp_, Read_t & read, vector<Lis_t> & lisIntervals, const align_opt & alnOptions, int & alnCount){
    ...
    while(alnCount < alnOptions.alignmentCount &&
        ...
        //sort this candidate region by query position
        sort(matchVector->begin()+begin,matchVector->begin()+end+1, compMatchesQuery);
        Alignment_t::Ptr alignment = extendAlignment(dp_, sa.S, lisIntervals[lisIndex].fw ? read.sequence : read.rcSequence,
...
bool isConcordantAlnToLis(const Alignment_t::Ptr mate1, const Lis_t& mate2, bool mate1isFirst, long editDist2, long M2length, const paired_opt& options){
    bool firstLeft = true;
...
void matchStrandOfPair(const SparseSA& sa,
    ...
    while(concordant < alnOptions.alignmentCount &&
        ...
        if(!lisIntervalsFM1[lisIndex].extended){
            ...
            //sort this candidate region by query position
            sort(matchVector->begin()+begin,matchVector->begin()+end+1, compMatchesQuery);
            Alignment_t::Ptr alignment = extendAlignment(dp_, sa.S, lisIntervalsFM1[lisIndex].fw ? mate1.sequence : mate1.rcSequence,
...
bool dpWindow(const Alignment_t::Ptr mate1, bool mate1isFirst, long editDist2, long M2length, const paired_opt& options,
    ...
    if(matches.size()>0){
        ...
        while(concordant < alnOptions.alignmentCount &&
            ...
            if(alignFirstMate || i == other.alignmentCount()){
                ...
                int end = lisIntervals[lisIndex].end;
                sort(matches.begin()+begin,matches.begin()+end+1, compMatchesQuery);
                Alignment_t::Ptr alignment = extendAlignment(dp_, sa.S, P, matches, begin, end, editDistBase, alnOptions);
                if(alignment!=NULL){
                    alignment->setLocalPos(sa);
                    if(!forward)
                        alignment->flag.set(4,true);
                    ...
                    if(output.editDist <= editDistOther){
                        //SUCCES!!!!!!!
                        Alignment_t::Ptr mate ( new Alignment_t() );
